...
impl Provider for $provider {
    ...
    async fn fetch_orchestration_item(
        ...
        tracing::debug!(target=$trace_target, instance_id=%instance_id, "Selected available instance");
        ...
        match lock_result {
            Ok(result) => {
                ...
                tracing::debug!(target=$trace_target, instance=%instance_id, rows_affected=affected, "Instance lock result");
                if affected == 0 {
                    // Failed to acquire lock (lock still held by another worker)
                    tracing::debug!(target=$trace_target, instance=%instance_id, "Failed to acquire instance lock (already locked)");
                    tx.rollback().await.ok();
                    return Ok(None);
                }
            }
            Err(e) => {
                tracing::debug!(target=$trace_target, instance=%instance_id, error=%e.to_string(), "Error acquiring instance lock");
                tx.rollback().await.ok();
        ...
        .await
        .map_err(|e| Self::sqlx_to_provider_error("fetch_orchestration_item", e))?;
        tracing::debug!(target=$trace_target, message_count=%messages.len(), rows_updated=%update_result.rows_affected(), instance=%instance_id, "Fetched and marked messages for locked instance");
    ...
    async fn ack_orchestration_item(
        ...
        for item in &orchestrator_items {
            ...
            tracing::debug!(target = $trace_target, instance=%instance, ?item, "enqueue orchestrator item in ack");
...
#[async_trait::async_trait]
impl ProviderAdmin for $provider {
    ...
    async fn delete_instance_bulk(&self, filter: InstanceFilter) -> Result<DeleteInstanceResult, ProviderError> {
        ...
        for instance_id in &instance_ids {
            ...
            // Atomic delete handles child/parent ordering internally.
